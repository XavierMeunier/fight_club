class Fighter < ApplicationRecord
  has_many :winned_fights, class_name: 'Fight', foreign_key: :winner_id, inverse_of: 'winner', dependent: :destroy
  has_many :lost_fights, class_name: 'Fight', foreign_key: :looser_id, inverse_of: 'looser', dependent: :destroy
  ...
  def add_experience_from_fight(status, given_punches, received_punches, rounds, opponent_id)
    ...
    level_gap = opponent.level - level
    ...
    if status == :winner
      ...
      experience_from_punches *= level_gap if level_gap.positive?
    else
      experience_from_rounds *= 2
      experience_from_rounds *= level_gap if level_gap.positive?
    end
  ...
  private
    ...
    def experience_to_level
      if LEVELS['levels'][level] <= experience
        self.level += 1
        self.experience = 0
        self.available_upgrade += 10
      end
